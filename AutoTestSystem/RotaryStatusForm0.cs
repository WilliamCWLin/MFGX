...
namespace AutoTestSystem
{
    ...
    public class RotaryStatusForm : Form
    {
        ...
        public void AppendLog(string message)
        {
            ...
            {
                logBox.BeginInvoke(new Action(delegate { AppendLogToUI(logLine); }));
            }
            else
            {
        ...
        private void CanvasPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            // 陰影效果（圓盤偏移）
            //Rectangle shadowRect = new Rectangle(circleRect.X + 6, circleRect.Y + 6, circleRect.Width, circleRect.Height);
            ...
            // 外框光暈
            //Rectangle glowRect = new Rectangle(circleRect.X - 10, circleRect.Y - 10, circleRect.Width + 20, circleRect.Height + 20);
            //using (SolidBrush glowBrush = new SolidBrush(Color.FromArgb(40, 100, 100, 255)))
            //{
            ...
            // 1. 畫高質感盤面底圖（盤面圖512x512居中）
            //int imgW = Rotate.Width;
            //int imgH = Rotate.Height;

            //int diameter1 = Math.Min(canvasPanel.Width, canvasPanel.Height) - 50;
            //int imgX = (canvasPanel.Width - diameter1) / 2;
            //int imgY = (canvasPanel.Height - diameter1) / 2;

            //g.DrawImage(Rotate, imgX, imgY, diameter1, diameter1);

            g.DrawEllipse(new Pen(Color.MidnightBlue, 4), circleRect);

            var bigFont = new Font("Arial", 14, FontStyle.Bold);
            var innerFont = new Font("Arial", 14, FontStyle.Bold);
