...
namespace AutoTestSystem.Base
{
    public abstract class IOBase : Manufacture.Equipment,IDisposable
    {
        ...
        public abstract bool SETIO(int portNum, int bit, bool output);

        public virtual bool GETDO(int portNum, int pos, ref bool status)
        {
            return true;
        }
        public virtual bool GETDIs(ref bool[] status)
        {
            return true;
        }
        
        public abstract bool GETIO(int portNum, int pos, ref bool status);
