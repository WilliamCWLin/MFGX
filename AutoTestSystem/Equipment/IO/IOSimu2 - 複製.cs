...
namespace AutoTestSystem.Equipment.IO
{
    public class IOSimu2 : IOBase
    {
        ...
        public override bool GETDIs(ref bool[] status)
        {
        ...
        public override bool Init(string strParamInfo)
        {
            try
            {
                _doStatus = new BitArray(IOCount, false); // 預設64點位，可根據需求調整
        _diStatus = new BitArray(IOCount, false); // 預設64點位，可根據需求調整
                return true;
            }
            catch (Exception e)
            {
