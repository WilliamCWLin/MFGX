...
namespace AutoTestSystem
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            GlobalNew.LogPath = $@"{GlobalNew.LOGFOLDER}\{GlobalNew.ProMode}\{GlobalNew.STATIONNAME}\{GlobalNew.FIXTURENAME}\{DateTime.Now.ToString("yyyyMMdd")}";
            ...
            // 4-3. 创建Csvdata 文件夹
            GlobalNew.csvLogPath    = $@"{GlobalNew.LOGFOLDER}\CsvData\{GlobalNew.TESTMODE}";
        ...
        private void MainForm_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if(jsonData["CurrentRecipePath"] == null)
                {
                    GlobalNew.CurrentRecipePath = "";
                    Logger.Error($"JSONData CurrentRecipePath = null");
                }
                else
                {
                    GlobalNew.CurrentRecipePath = $"{jsonData["CurrentRecipePath"].ToString()}";
                }
                if (jsonData["CurrentDevicePath"] == null)
                {
                    GlobalNew.CurrentDevicePath = "";
                    Logger.Error($"JSONData CurrentRecipePath = null");
                }
                else
                {
                    GlobalNew.CurrentDevicePath = $"{jsonData["CurrentDevicePath"].ToString()}";
                }
                
                if (GlobalNew.ProtreeON == "1")
                    GlobalNew.DeviceListPath = $@"{GlobalNew.CurrentDevicePath}";
                else
            ...
            if (GlobalNew.ProtreeON == "1")
            {
                ret = ProTreeView.ProTreeView.Load_Devices(GlobalNew.CurrentDevicePath, GlobalNew.Devices);     //! ProTreeView load device
                Logger.Info($"Load device path. {GlobalNew.CurrentDevicePath}");
            }
            else
            {
                ret = ProTreeView.ProTreeView.Load_Devices(GlobalNew.DeviceListPath, GlobalNew.Devices);     //! ProTreeView load device
            }
            ...
            try
            {
                switch (testStatus)
                {
                    case TestStatus.START:
                        ...
                        SetButton(this.ConfigureBtn, "", BtnLogin.BackColor, false);
                ...
                finally
                {
                    try
                    {
                        if (testStatus != TestStatus.START)
                        {
                            ...
                            SetTextBox(textBox2);
                            SetTextBox(textBox1);
                            SetButton(this.ConfigureBtn, "", BtnLogin.BackColor);
        ...
        public void StartInit(DUT_BASE tempDUT)
        {
            ...
            tempDUT.TestInfo.AddTestStep("Program Info", "Test Program V2.50.15");
            tempDUT.TestInfo.AddTestStep("Program Info", "Test config File version 0.0.1.0");
            //tempDUT.TestInfo.AddTestStep("Test Summary", "Total Test Time: 7119 secs");
            tempDUT.TestInfo.AddTestStep($"Test Summary", "SN: {PSN}");
            if (tempDUT != null)
            {
                GlobalNew.g_datacollection.SetMoreProp("PSN", PSN);
                tempDUT.DataCollection.SetMoreProp("ProductSN", SN);
                tempDUT.DataCollection.SetMoreProp("WorkID", GlobalNew.CurrentUser);
                tempDUT.DataCollection.SetMoreProp("StartTime", startDateTime.ToString());
            }
        }
        ...
        {         
            ...
            GlobalNew.g_datacollection.SetMoreProp("EndTime", endDateTime.ToString());
            GlobalNew.g_datacollection.SetMoreProp("Result", TestResult);
            if (tempDUT != null)
            {
                tempDUT.DataCollection.SetMoreProp("EndTime", endDateTime.ToString());
        ...
        public void Total_TestTime(DUT_BASE tempDUT)
        {
            ...
            elapsedTime = startDateTime - endDateTime;
            ...
            try
            {
                ...
                for (int i = 0; i < GlobalNew.cycles; i++)
                {
                    ...
                    if (stationStatus)
                    {
                        ...
                        WriteCSVResult(HeaderFilePath, CSVFile_path, GlobalNew.g_datacollection);//Total log in csv
                        WriteCSVResult(HeaderFilePath, CSVFile_path_PASS, GlobalNew.g_datacollection);
                    ...
                    else
                    {
                        ...
                        WriteCSVResult(HeaderFilePath, CSVFile_path, GlobalNew.g_datacollection);//Total log in csv
                        WriteCSVResult(HeaderFilePath, CSVFile_path_FAIL, GlobalNew.g_datacollection);
        ...
        //       輸出Report                                                                 //
        //////////////////////////////////////////////////////////////////////////////////////
        static void WriteCSVResult(string HeaderFilePath, string csvFilePath, DataCollection g_datacollection)
        {            
                ...
                foreach (var header in headers)
                {
                    ...
                    //foreach (var data in dataList)
                    {
                        ...
                        try
                        {
                            ...
                            Logger.Info($"GetMoreProp:{header},Value:{temp}");
                            if (temp != "")
                            {
                                fielddata = temp;
                            }
            ...
            try
            {
                ...
                else
                {
                    if (!textbox.IsDisposed)
                    {
                        ...
                        textbox.SaveFile($"{GlobalNew.txtLogPath}\\{path}", a);                  //!保存测试log
        ...
        private void CopyLogToServer(string mapDrive, string logfile)
        {
            try
            {
                ...
                string destPath = $@"{GlobalNew.LOGSERVER}\{GlobalNew.STATIONNAME}\{GlobalNew.FIXTURENAME}\{DateTime.Now.ToString("yyyyMMdd")}\{logfile}";
        ...
        private void ConfigureBtn_Click(object sender, EventArgs e)
        {
            ...
            //LoginForm CG_Form = new LoginForm();
            //DialogResult res  = CG_Form.ShowDialog();
            //if (res == DialogResult.OK)
            {
                //Manufacture.Global_Memory.UserLevel = 5;
            ...
            foreach (var value in GlobalNew.Devices.Values)
            {
                if (value is DUT_BASE)
                {
                   ...
                   DataGridView dataGridView = temp.DataGridView;
                    ...
                    dataGridView.Columns["Result"].FillWeight = 12;
                ...
                if (n.Tag is Container_MainThread == true)
                {
                    ...
                    if (T.isRunning == 0)
                    {
                        ...
                        if (tempDUT == null)
                        {
                            ...
                            //SetButton(this.bt_errorCode, tempDUT.DataCollection.GetMoreProp("ProductSN"), Color.Red);
                            End_TestTime("FAIL", tempDUT);
                            Total_TestTime(tempDUT);
                        ...
                        object[] context = new object[] { MainProTreeView, T.FailContinue, tempDUT , TestResult };
                        ...
                        ta.ContinueWith(t =>
                        {
                            //tempDUT.TestInfo.ExportToFile("D:\\xxx.txt");
                            //tempDUT.TestInfo.ExportToPdf("D:\\xxx.pdf", tempDUT.TestInfo.GetTestSections());
                            ...
                            if (result)
                            {
                                ...
                                SetTestStatus(TestStatus.PASS);
                                End_TestTime("PASS", tempDUT);
                                Total_TestTime(tempDUT);
                                WriteCSVResult(HeaderFilePath, CSVFile_path, tempDUT.DataCollection);//Total log in csv
                                WriteCSVResult(HeaderFilePath, CSVFile_path_PASS, tempDUT.DataCollection);
                            ...
                            else
                            {
                                ...
                                Total_TestTime(tempDUT);
                                WriteCSVResult(HeaderFilePath, CSVFile_path, tempDUT.DataCollection);//Total log in csv
                                WriteCSVResult(HeaderFilePath, CSVFile_path_FAIL, tempDUT.DataCollection);
        ...
        private void MainProTreeNodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                int len = MainlogRichTextBox.Text.IndexOf(e.Node.Text.Substring(2).Trim());
                if (len > 0)
                {
                    //! 光标跳到指定行
